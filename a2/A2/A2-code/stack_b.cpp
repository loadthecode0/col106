...
int Stack_B :: pop() {
    if (size>0) {
        ...
        if (capacity >= 4096) { //shrink by 4 allowed
            ...
            if (size > 257) {
                return stk [--size]; //edit: shrink to 1024 only if size <=256, so that sudden jerks don't happen at 1024 size
            } else { //size = 257 now, after pop, shrink to be 1024-sized
                int poppedElt = stk[size-1];
                try
                {

...
//      _CBIGNUM_TOATMP     разрешить метод toatmp().
//      _CBIGNUM_CONSTCAST  разрешить const cast.
...
/*
  void  create ()               // Тестовый метод для ненормализованного 0.
  {
...
//      Конструкторы.
#ifndef _CBIGNUM_DEF_ALLOC
  cBigNumber ()                 { e = _stub; len = 0; } // По умолчанию (0).
#else //_CBIGNUM_DEF_ALLOC
  cBigNumber ()                 { create();   }         // По умолчанию (0).
#endif//_CBIGNUM_DEF_ALLOC
...
//      Обнуление с оптимизацией памяти.
...
//      Операнды должны иметь одинаковый знак. Делитель должен иметь таблицу
//      сдвигов и не должен перекрываться с буфером результата.
...
//      Операнды должны иметь одинаковый знак, не должны перекрываться друг
//      с другом и буфером результатом. Делитель должен иметь таблицу сдвигов.
...
static  void  maskdiv0  (int);          // Маска деления на 0.
static  int   testdiv0  ();             // Признак деления на 0.

//      Доступ к статическим буферам (не рекомендуется).
#ifndef _CBIGNUM_MT
...
//================================================
//      Служебные переменные класса (не применять).
//================================================
...
{
#ifndef _CBIGNUM_DEF_ALLOC
  e = _stub; len = 0;                                   // Обнуление полей.
#else //_CBIGNUM_DEF_ALLOC
  create();                                             // Создание числа.
#endif//_CBIGNUM_DEF_ALLOC

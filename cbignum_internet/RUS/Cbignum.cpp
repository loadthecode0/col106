...
//================================================
//      Заглушка для числа с начальным значением 0.
//================================================
...
//================================================
//      Служебные переменные класса.
//================================================

size_t  cBigNumberMaskDiv0;                     // Маска деления на 0.

//      Подлежат исключению - не для реентерабельного или многопоточного кода.
#ifndef _CBIGNUM_MT
...
//================================================
//      Реализация операций над предварительно
//      подготовленными операндами.
//================================================
...
//      Для сокращения числа делений применяется деление на
//      коэффициенты powradix, полученные путем последовательного
//      возведения основания radix в квадрат.
//
...
//      укладывающиеся в разрядную сетку типа long. Эти числа
//      преобразуются в строку путем последовательного деления
//      на radix.
//
...
//      находится младшее частное. Для старшего частного в
//      соответствующем по номеру элементе массива kdiv всегда
//      находится 0. Первоначально массив частных пуст.
//
//      АЛГОРИТМ
...
#ifdef  _CBIGNUM_SMALL_DIV                      // и остатка в mod_.
...
//      он делится на соответствующий коэффициент.
      ...
      {
        assert (kdiv [np - 1] == 0);            // Контроль.
        kdiv [--np] = 1;                        // Установка признака.
#ifdef  _CBIGNUM_SMALL_DIV
        if (np <= (size_t)(_CBIGNUM_SMALL_DIV > 1)) {
          div [np].setdivmod (mod_, *powr [np]);
        } else                                  // Запись частного в div
#endif//_CBIGNUM_SMALL_DIV                      // и остатка в mod_.
...
//      если оно там есть (это так всегда, кроме случая npmax == 0).
      ...
      {
        long lnum = (long) mod_.loword();       // Короткий остаток.
...
//================================================
//      Реализация независимого вывода.
//================================================

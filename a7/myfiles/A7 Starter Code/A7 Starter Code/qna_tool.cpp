...
Node *BMScore(vector<vector<string>> Q, Trie *T, int k)
{
    ...
    for (auto v : Q)
    {
        ...
        if (bmNode != NULL)
        {
            for (auto p : bmNode->existence) // refers to each paragraph in which query q is present and update its score
            {
                ...
                // for (auto nd : para)
                // {
                //     if (nd->paraNum == p->paragraph && nd->book_code == p->book_code && nd->page == p->page)
                //     {
                //         temp = nd;
                //     }
                // }
                ...
                // cout << "book no  " << b_num << endl;
                //  cout << q << endl;
                //  cout << "para no  " << p_num << endl;
                //  cout << "score " << table[b_num][pg_num][p_num] << "\n";
                //  }
                //  else
                //  {
                ...
                //      temp->Score = score;
                //      temp->page = p->page;
                //      temp->book_code = p->book_code;
                //      temp->paraNum = p->paragraph;
                //      para.push_back(temp);
                //  }
            }
        }
    ...
    // cout << "          " << (to_be_inserted.size() == to_be_sorted.size()) << endl;
    long long int sz = to_be_sorted.size();
    H.buildHeap(to_be_inserted, k);
    // for (int i = 0; i < H.heap.size(); i++)
    // {
    //     Master_para_node *p = H.heap[i];
    //     cout << p->para_score << endl;
    // }
    if (H.is_empty())
    {
        return NULL;
    ...
    for (long long int j{k}; j < sz; j++)
    {
        ...
        {
            // if ()
            // cout << H.heap[0]->para_score << endl;
            // cout << to_be_inserted[j]->para_score << endl;
    ...
    while (i < k)
    {
        ...
        H.deleteMin();
        // cout << "score " << min_element->para_score << endl;
...
{ ////aastha's function would make the vector of string

    Trie *T = this->personal_dictionary->dictionary;

    return BMScore(question, T, k);
}
...
QNA_tool::QNA_tool()
{
    ...
    if (read_me.is_open())
    {
        ...
        while (getline(read_me, s))
        {
            ...
            // Split the line into string and number using a comma as the delimiter
            getline(flow, word, ',');
            flow >> value;
            int temp = 0;
...
{
    int num_word = 0;
    personal_dictionary->insert_sentence(book_code, page, paragraph, sentence_no, sentence, 1.0, num_word);
...
Node *QNA_tool::get_top_k_para(string question, int k)
{
    ...
    for (int i = 0; i < question.size(); i++)
    {
        if (is_separator_v2(separators, question[i]))
        {
            if (word != "")
            {
                ...
                long double score = (1 + personal_dictionary->get_word_count(word)) / (1 + global_dictionary->get_word_count(word));
    ...
    for (int i = 0; i < query_words.size(); i++)
    {
        ...
        if (refer != nullptr)
        {
            for (int j = 0; j < refer->existence.size(); j++)
            {
                ...
                // if (ScoreStore[refer->existence[j]->book_code][refer->existence[j]->page][refer->existence[j]->paragraph] > 100)
                // cout << ScoreStore[refer->existence[j]->book_code][refer->existence[j]->page][refer->existence[j]->paragraph] << "score store" << endl;
                ...
                else
                {
                    ...
                    to_be_sorted.push_back(refer->existence[j]);
                }
            }
        }
    }
    ...
    while (i < k)
    {
        ...
        Master_para_node *min_element = H.minElem();
        cout << min_element->para_score << endl;
        H.deleteMin();
        // cout << "score " << min_element->para_score << endl;
...
// note : see about the lon lon int and double ...
...
vector<vector<string>> generateInputVector()
{
    ...
    cout << "queryinput generated\n";

    return input;
}

Node *QNA_tool::query(string question, string filename)
{
    // Implement your function here
    ...
    // process now
    int k = 5;
    Node *res = get_top_k_para2(inputs, k);
    cout << "hello2\n";
...
std::string QNA_tool::get_paragraph(int book_code, int page, int paragraph)
{
    ...
    std::string filename = "books/mahatma-gandhi-collected-works-volume-";
...
// int main()
// {
//     QNA_tool *my_ai = new QNA_tool();
//     string s = my_ai->get_paragraph(1, 0, 7);
//     cout << s;
